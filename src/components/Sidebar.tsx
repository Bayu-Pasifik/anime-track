            ...
            <div className="sidebar bg-gray-100 mx-4 mt-4 rounded-xl h-auto p-4 shadow-lg overflow-x-auto" key={animeDetail.mal_id}>
                <div className="w-full h-auto lg:grid lg:grid-cols-1 gap-4 flex flex-row flex-wrap lg:flex-col">
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        ...
                        <p className="text-md font-medium text-gray-500">{animeDetail.type || 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Episodes</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.episodes || 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                    ...
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        ...
                        <p className="text-md font-medium text-gray-500">{animeDetail.status || 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Start Date</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.aired?.from ? animeDetail.aired.from.replace("T00:00:00+00:00", "") : 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">End Date</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.aired?.to ? animeDetail.aired.to.replace("T00:00:00+00:00", "") : 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Season</p>
                        <p className="text-md font-medium text-gray-500">{(animeDetail.season || animeDetail.year) ? `${animeDetail.season} ${animeDetail.year}` : 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Popularity</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.popularity || 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Favorites</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.favorites || 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Studios</p>
                        {animeDetail.studios?.length > 0 ? animeDetail.studios.map((studio, index) => (
                            <p key={index} className="text-md font-medium text-gray-500">{studio.name}</p>
                        )) : <p className="text-md font-medium text-gray-500">Undefined</p>}
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Producers</p>
                        {animeDetail.producers?.length > 0 ? animeDetail.producers.map((producer, index) => (
                            <p key={index} className="text-md font-medium text-gray-500">{producer.name}</p>
                        )) : <p className="text-md font-medium text-gray-500">Undefined</p>}
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Source</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.source || 'Undefined'}</p>
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Genres</p>
                        {animeDetail.genres?.length > 0 ? animeDetail.genres.map((genre, index) => (
                            <p key={index} className="text-md font-medium text-gray-500">{genre.name}</p>
                        )) : <p className="text-md font-medium text-gray-500">Undefined</p>}
                    </div>
                    <div className="flex-shrink-0 w-full lg:w-auto">
                        <p className="text-xl font-medium text-blue-600">Rating</p>
                        <p className="text-md font-medium text-gray-500">{animeDetail.rating || 'Undefined'}</p>
                    </div>
                </div>
            </div>
